...
<div class="container mx-auto px-4 py-8">
    ...
    @foreach($testResults as $index => $testResult)
        <a href="{{ route('testResult.show', ['id' => $testResult->id]) }}"
           class="block bg-white rounded-lg shadow-md my-3 transform transition-transform hover:scale-105 hover:bg-gray-100 hover:shadow-lg hover:font-bold">
            <div class="p-6 flex">
                <div class="text-2xl">{{ $index + 1 }}</div>
                <div class="ml-2 text-2xl">{{ $testResult->created_at }}</div>
                <hr class="my-2">
            </div>
        </a>
